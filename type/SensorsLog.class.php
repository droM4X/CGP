...
class Type_SensorsLog extends Type_Base {
	...
	function rrd_gen_graph() {
		...
		if ($this->graph_minmax) {
			...
			foreach ($sources as $source) {
				...
				$rrdgraph[] = sprintf('LINE3:max_%s#%s', crc32hex($source), $this->get_faded_color($color));
				$rrdgraph[] = sprintf('LINE3:min_%s#%s', crc32hex($source), $this->get_faded_color($color));
			}
		}
